...
public class FirstPersonDrawer extends DefaultViewer {
	...
	MazePanel gc ; // the graphics object for the buffer image this class draws on
	 ...
	 */
	@Override
	public void redraw(MazePanel gc, int state, int px, int py, int view_dx, int view_dy, int walk_step, int view_offset, RangeSet rset, int ang) {
		...
		gc.setColorDarkGray();
		gc.fillRect(0, view_height/2, view_width, view_height);
		
		// set color to white and draw what ever can be seen from the current position
		gc.setColorWhite();
		//maze.panel.col.setColor(8000000);
		...
		//gc.setFontMedium();
		//gc.setFontMetrics();
		//gc.centerString(gc, gc.fm, "PRESS 'v' TO SAVE A COPY OF THE MAZE", 685);
	...
	private boolean boundingBoxIsVisible(int ymax, int ymin, int xmin, int xmax) {
		...
		//adjusted to become an integer array
		int[] p = {x1, x2};
